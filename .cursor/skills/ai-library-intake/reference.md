...
- Frame download defaults to `--extractor-args youtube:player_client=android,web` (override with `LIBRARY_INTAKE_YTDLP_EXTRACTOR_ARGS`) to avoid common YouTube HTTP 403 / SABR-only format failures.
- Soft-fail reasons SHOULD include typed-error fields such as `stderr=` from yt-dlp/ffmpeg.
...
- MUST set exactly one `does` from the taxonomy Does table (INDEX skim axis).
- MUST use only allowed tags from taxonomy (detail; not INDEX skim).
...
- MUST set exactly one `does` from the taxonomy Does table.
...
|------|-------|----------|---------|------|-----|---------|
| block-3d | Block 3D | 3d | Text-to-3D cleanup for product meshes. | 3d, open, local | local | open |
| voicemem | VoiceMem | agents-memory | Local voice memory for companion agents. | memory, open, local, audio | local | open |
```

Also regenerate `library/<category>/INDEX.md` and `library/additions/YYYY-Www.md` (per-section tables inside each week). Week open/closed status is derived from each entry's `added` ISO week.
...
5. Write → `library/agents-memory/voicemem.md`; `library-intake index refresh`.
...
3. On `y` → WebSearch enrich (required) → attach media if any → write → `index refresh`; then next candidate.
